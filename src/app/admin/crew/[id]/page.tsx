  ...
  useEffect(() => {
    const fetchCrewMember = async () => {
      try {
        ...
        setError("Failed to load crew member");
        console.error(err);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold mt-2">Edit Crew Member</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-[var(--theme-surface)] p-6 rounded-xl border border-[var(--theme-border)]">
        ...
        <div>
          ...
          <input type="text" value={formData.certifications} onChange={(e) => setFormData({ ...formData, certifications: e.target.value })} className="w-full px-4 py-2 bg-[var(--theme-bg)] border border-[var(--theme-border)] rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent" />
        </div>
        ...
        <div className="flex gap-4">
          <button type="submit" disabled={saving} className="px-6 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition-colors disabled:opacity-50">
